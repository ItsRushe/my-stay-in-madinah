  ...
  return (
    <>
      <section className="px-6 md:px-12 max-w-[90rem] mx-auto mb-20 relative">
        <div className="absolute right-10 top-4 z-10 hidden md:flex items-center gap-2 bg-white/80 backdrop-blur px-4 py-2 rounded-full shadow-sm text-ink/60 text-xs font-medium uppercase tracking-wider pointer-events-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-auto md:h-[600px]">
          <div className="md:col-span-2 rounded-2xl overflow-hidden h-[300px] md:h-full bg-gray-100 group relative cursor-zoom-in" onClick={() => setSelectedImage(images[0])}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-1 gap-4 md:h-full">
            <div className="rounded-2xl overflow-hidden h-[200px] md:h-full bg-gray-100 group relative cursor-zoom-in" onClick={() => setSelectedImage(images[1])}>
            ...
            <div className="rounded-2xl overflow-hidden h-[200px] md:h-full bg-gray-100 group relative cursor-zoom-in" onClick={() => setSelectedImage(images[2])}>
        ...
        <div className="mt-4 rounded-2xl overflow-hidden h-[300px] md:h-[500px] bg-gray-100 group relative cursor-zoom-in" onClick={() => setSelectedImage(images[3])}>
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center backdrop-blur-sm" onClick={() => setSelectedImage(null)}>
          ...
          <img src={selectedImage} alt="Full Screen Room View" className="max-w-[95vw] max-h-[90vh] object-contain shadow-2xl rounded-sm" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </>
  );
