      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 32px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #E5E2DA', padding: '28px 32px', marginBottom: '32px' }}>
          <div style={{ marginBottom: '20px' }}>
            <h2 style={{ margin: '0 0 4px', fontSize: '15px', fontWeight: 600, color: '#1B2420' }}>This Week's Availability</h2>
            <p style={{ margin: 0, fontSize: '12px', color: '#9a9a90' }}>Next 7 days at a glance</p>
          </div>
          <BookingCalendar bookings={allBookings} rooms={allRooms} days={7} />
        </div>

        {/* Rooms */}
        <div style={{ background: '#fff', border: '1px solid #E5E2DA', padding: '28px 32px', marginBottom: '32px' }}>
          <div style={{ marginBottom: '20px' }}>
            <h2 style={{ margin: '0 0 4px', fontSize: '15px', fontWeight: 600, color: '#1B2420' }}>Rooms</h2>
            <p style={{ margin: 0, fontSize: '12px', color: '#9a9a90' }}>Toggle active/coming soon · edit rate · click View for room bookings</p>
          </div>
          <RoomManager rooms={allRooms as any} />
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                ...
                <tbody>
                  {allBookings.map((booking, i) => {
                    ...
                    const roomData = allRooms.find(r => r.id === booking.room_id);
                    const roomLabel = roomData?.room_number ? `#${roomData.room_number}` : roomName;
                    ...
                    return (
                      <tr key={booking.id} style={{ borderBottom: '1px solid #F0EDE6', background: i % 2 === 0 ? '#fff' : '#FDFCFA' }}>
                        ...
                        <td style={{ padding: '14px 20px', color: '#1B2420', fontWeight: 500, whiteSpace: 'nowrap' }}>{roomLabel}</td>
